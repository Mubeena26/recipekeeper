...
class _SnacksState extends State<Snacks> {
  late List<RecipeModel> snacksRecipes = [];
  ...
  Future<void> getRecipeByCategory() async {
    ...
    await Future.forEach(recipeBox.values, (RecipeModel recipe) {
      if (recipe.categories == 'Snacks') {
        recipes.add(recipe);
      }
    });
    setState(() {
      snacksRecipes = recipes;
    });
  }
  ...
  //   snacksRecipes = await getRecipesByCategory('Snacks');
  ...
  Widget build(BuildContext context) {
    ...
    return ValueListenableBuilder(
        ...
        builder: (context, Box<RecipeModel> favoritesBox, _) {
          ...
          return snacksRecipes.isEmpty
              ? Center(
                  child: Text('No recipes found in Snacks category'),
                )
              : ListView.separated(
                  itemBuilder: (ctx, index) {
                    final RecipeModel recipe = snacksRecipes[index];
                  ...
                  itemCount: snacksRecipes.length,
                );
        });
  }
}
